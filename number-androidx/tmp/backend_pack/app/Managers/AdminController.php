...
class AdminController extends Manager
{
    ...
    public function listUsersApi($request, $response)
    {
        $queryParams = $request->getQueryParams();
        $search = $queryParams['search'] ?? '';

        if (!empty($search)) {
            $sql = "SELECT memberid, username, realname, surname, vipcode FROM membertb 
                    WHERE username LIKE :s OR realname LIKE :s OR surname LIKE :s OR memberid = :exact
                    LIMIT 50";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([':s' => "%$search%", ':exact' => $search]);
            ...
            $sql = "SELECT memberid, username, realname, surname, vipcode FROM membertb 
                    ORDER BY memberid DESC LIMIT 50";
            $stmt = $this->db->query($sql);
        }

        $users = $stmt->fetchAll(\PDO::FETCH_ASSOC);
        $response->getBody()->write(json_encode(['status' => 'success', 'users' => $users], JSON_UNESCAPED_UNICODE));
        return $response->withHeader('Content-Type', 'application/json');
    }

    public function updateUserStatusApi($request, $response)
    {
        $body = $request->getParsedBody();
        if (empty($body)) {
            $raw = file_get_contents('php://input');
        ...
        if (empty($memberid) || empty($vipcode)) {
            $response->getBody()->write(json_encode(['status' => 'error', 'message' => 'Missing data']));
            return $response->withHeader('Content-Type', 'application/json');
        }

        $sql = "UPDATE membertb SET vipcode = :vip WHERE memberid = :mid";
        $stmt = $this->db->prepare($sql);
        $success = $stmt->execute([':vip' => $vipcode, ':mid' => $memberid]);

        if ($success) {
            $response->getBody()->write(json_encode(['status' => 'success', 'message' => 'สถานะถูกเปลี่ยนเป็น ' . $vipcode]));
        } else {
