...
class NotificationController extends Manager
{
    ...
    {
        $cleanedBagCount = 0;
        $cleanedAuspiciousCount = 0;
        $cleanedInauspiciousCount = 0;

        try {
            // 1. Global Bag Color Cleanup (Delete past ages)
            $stmtUsers = $this->db->query("SELECT memberid, birthday FROM membertb WHERE birthday IS NOT NULL");
            $users = $stmtUsers->fetchAll(PDO::FETCH_OBJ);
            $pm = new PersonManager();
            foreach ($users as $u) {
                if (empty($u->birthday))
                    continue;
                $ages = $pm->age(strtotime($u->birthday), time());
                $age = $ages['year'] ?? 0;
                if ($age > 0) {
                    $stmt = $this->db->prepare("DELETE FROM bagcolortb WHERE memberid = :mid AND age < :age");
                    $stmt->execute([':mid' => $u->memberid, ':age' => $age]);
                    $cleanedBagCount += $stmt->rowCount();
                }
            }

            // 2. Clear Expired Assignments (Hard Delete)
            $stmt1 = $this->db->prepare("DELETE FROM user_auspicious_assign WHERE expires_at IS NOT NULL AND expires_at < NOW()");
            $stmt1->execute();
            $cleanedAuspiciousCount = $stmt1->rowCount();

            $stmt2 = $this->db->prepare("DELETE FROM user_inauspicious_assign WHERE expires_at IS NOT NULL AND expires_at < NOW()");
            $stmt2->execute();
            $cleanedInauspiciousCount = $stmt2->rowCount();
        ...
        return [
            'cleaned_bag_colors' => $cleanedBagCount,
            'cleaned_auspicious' => $cleanedAuspiciousCount,
            'cleaned_inauspicious' => $cleanedInauspiciousCount,
            'total_cleaned' => ($cleanedBagCount + $cleanedAuspiciousCount + $cleanedInauspiciousCount)
        ];
    }
