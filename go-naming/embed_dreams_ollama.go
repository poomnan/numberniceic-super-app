...
func main() {
	...
	// Prepare column for 768 dimensions
	_, err = db.Exec("ALTER TABLE dreams DROP COLUMN IF EXISTS meaning_vector; ALTER TABLE dreams ADD COLUMN meaning_vector vector(768);")
	if err != nil {
		log.Fatal(err)
	}

	rows, err := db.Query("SELECT dream_id, dream_keyword, dream_meaning FROM dreams")
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var id int
		var keyword, meaning string
		if err := rows.Scan(&id, &keyword, &meaning); err != nil {
			log.Printf("Scan error: %v", err)
			continue
		}

		textToEmbed := fmt.Sprintf("%s: %s", keyword, meaning)
		...
		if err != nil {
			...
			fmt.Printf("Success [%d] (Dim: %d)\n", id, len(vector))
		}

		time.Sleep(100 * time.Millisecond)
	}

	fmt.Println("All dreams embedded with Ollama!")
}
