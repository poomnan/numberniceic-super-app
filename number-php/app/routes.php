...
// --- Admin: Rengyam Tag Meanings (Editable Astrology Terms) ---
// This powers ThaiCalendarHelper::getTagMetaFromDb() so the app can show editable tag descriptions.
// CRUD here is intentionally "web admin style" consistent with existing features (session admin check + views).
$app->get('/web/admin/rengyam-tags', function ($request, $response) use ($container) {
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
    $user = $_SESSION['user'] ?? null;
    ...
    $db = $container->get('db');
    // Ensure table exists (idempotent) to avoid a broken admin screen.
    $db->exec("
    ...
    if ($q === '') {
        $stmt = $db->query("SELECT * FROM rengyam_tag_meanings ORDER BY is_active DESC, tag_name ASC");
        $rows = $stmt->fetchAll(\PDO::FETCH_OBJ);
    } else {
        $like = '%' . $q . '%';
        $stmt = $db->prepare("
            SELECT * FROM rengyam_tag_meanings
            WHERE tag_name LIKE :q OR source_label LIKE :q OR short_description LIKE :q
            ORDER BY is_active DESC, tag_name ASC
        ");
        $stmt->execute([':q' => $like]);
        $rows = $stmt->fetchAll(\PDO::FETCH_OBJ);
    }

    return $container->get('view')->render($response, 'web_admin_rengyam_tags.php', [
        'user' => $user,
        'rows' => $rows,
        'q' => $q,
    ]);
...
$app->get('/web/admin/rengyam-tags/create', function ($request, $response) use ($container) {
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
    $user = $_SESSION['user'] ?? null;
    ...
    return $container->get('view')->render($response, 'web_admin_rengyam_tag_form.php', [
        'user' => $user,
        'mode' => 'create',
        'row' => null,
        'error' => null,
...
$app->get('/web/admin/rengyam-tags/edit/{id}', function ($request, $response, $args) use ($container) {
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
    $user = $_SESSION['user'] ?? null;
    ...
    $id = (int) ($args['id'] ?? 0);
    if ($id <= 0) {
        return $response->withHeader('Location', '/web/admin/rengyam-tags')->withStatus(302);
    }

    $stmt = $db->prepare("SELECT * FROM rengyam_tag_meanings WHERE id = :id LIMIT 1");
    $stmt->execute([':id' => $id]);
...
$app->post('/web/admin/rengyam-tags/store', function ($request, $response) use ($container) {
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
    $user = $_SESSION['user'] ?? null;
    ...
    if ($tagName === '' || $sourceLabel === '' || $shortDesc === '') {
        return $container->get('view')->render($response, 'web_admin_rengyam_tag_form.php', [
            'user' => $user,
            'mode' => 'create',
            'row' => (object) [
            ...
            'error' => 'กรุณากรอกข้อมูลให้ครบ (ชื่อแท็ก/สำนัก/คำอธิบาย)',
        ]);
    }
    ...
    $stmt = $db->prepare("
        ...
        VALUES (:tag_name, :source_label, :short_description, :is_active)
...
$app->post('/web/admin/rengyam-tags/update/{id}', function ($request, $response, $args) use ($container) {
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
    $user = $_SESSION['user'] ?? null;
    ...
    if ($tagName === '' || $sourceLabel === '' || $shortDesc === '') {
        ...
        // Override with attempted values for a better UX.
        $row->tag_name = $tagName ?: $row->tag_name;
        $row->source_label = $sourceLabel ?: $row->source_label;
        $row->short_description = $shortDesc ?: $row->short_description;
...
$app->get('/web/admin/rengyam-tags/toggle/{id}', function ($request, $response, $args) use ($container) {
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
    $user = $_SESSION['user'] ?? null;
    ...
    $db = $container->get('db');
    $stmt = $db->prepare("UPDATE rengyam_tag_meanings SET is_active = IF(is_active=1,0,1) WHERE id = :id");
    $stmt->execute([':id' => $id]);
