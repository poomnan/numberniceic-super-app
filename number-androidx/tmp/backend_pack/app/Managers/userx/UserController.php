    ...
    public function vipCode($request, $response)
    {
        ...
        if ($code != '') {
            ...
            if (is_object($object)) {
                $resData = array(
                    'member' => 'vip',
                    'vipcode' => array('vipid' => $object->vipid, 'vipcode' => $object->vipcode, 'userdetial' => $object->userdetial, 'viptype' => $object->viptype, 'vipstatus' => $object->vipstatus)
                );
                $response->getBody()->write(json_encode($resData));
                return $response->withHeader('Content-Type', 'application/json');
            }
            
            // Check secretcode table ONLY if active
            ...
            if (is_object($object2)) {
                ...
                $resData = array(
                    'member' => 'vip',
                    'vipcode' => array('vipid' => $object2->codeid, 'vipcode' => $object2->codename, 'userdetial' => 'Admin Secret Code', 'viptype' => $object2->codetype, 'vipstatus' => 'used')
                );
                $response->getBody()->write(json_encode($resData));
                return $response->withHeader('Content-Type', 'application/json');
            }
