...
class ChangeNumAssignF : Fragment() {
    ...
    companion object {
        fun newInstance(): ChangeNumAssignF {
            val args = Bundle()
            val fragment = ChangeNumAssignF()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        txtSearchTitle.text = "ค้นหาลูกค้าเพื่อส่งขั้นตอนการเปลี่ยนแปลง"
    ...
    private fun displayUsers(users: List<UserZ>) {
        val adapter = FindUserAdapter(users, object : FindUserAdapter.OnItemClickListener{
            override fun itemClick(userZ: UserZ) {
                // Redirect to ChangeNumAssignPickerAct
                val intent = Intent(context, ChangeNumAssignPickerAct::class.java)
