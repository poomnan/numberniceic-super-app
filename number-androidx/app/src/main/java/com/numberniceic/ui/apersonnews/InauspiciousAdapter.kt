...
class InauspiciousAdapter(private val items: List<InauspiciousData>) :
    RecyclerView.Adapter<InauspiciousAdapter.ViewHolder>() {

    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        val txtTitle: TextView = view.findViewById(R.id.txt_title)
        val txtDesc: TextView = view.findViewById(R.id.txt_desc)
        val img: ImageView = view.findViewById(R.id.img_inauspicious)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_inauspicious, parent, false)
        return ViewHolder(view)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val item = items[position]
        // Show the content in the colored title field and hide the detail field
