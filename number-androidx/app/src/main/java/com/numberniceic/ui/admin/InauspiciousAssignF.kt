...
import com.numberniceic.ui.merit.MeritWebViewAct
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class InauspiciousAssignF : Fragment() {
    ...
    companion object {
        fun newInstance(): InauspiciousAssignF {
            val args = Bundle()
            val fragment = InauspiciousAssignF()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        txtSearchTitle.text = "ค้นหาลูกค้าเพื่อจัดการวันอัปมงคล"
    ...
    private fun displayUsers(users: List<UserZ>) {
        val adapter = FindUserAdapter(users, object : FindUserAdapter.OnItemClickListener{
            override fun itemClick(userZ: UserZ) {
                // Open Native Form
                val intent = Intent(context, InauspiciousFormAct::class.java)
