...
class MeritAssignF : Fragment() {
    ...
    companion object {
        fun newInstance(): MeritAssignF {
            val args = Bundle()
            val fragment = MeritAssignF()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        txtSearchTitle.text = "ค้นหาลูกค้าเพื่อส่งวิธีทำบุญ"
    ...
    private fun displayUsers(users: List<UserZ>) {
        val adapter = FindUserAdapter(users, object : FindUserAdapter.OnItemClickListener{
            override fun itemClick(userZ: UserZ) {
                // Redirect to MeritAssignPickerAct
                val intent = Intent(context, MeritAssignPickerAct::class.java)
