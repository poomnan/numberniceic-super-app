...
class SpellAssignF : Fragment() {
    ...
    companion object {
        fun newInstance(): SpellAssignF {
            val args = Bundle()
            val fragment = SpellAssignF()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val root = inflater.inflate(R.layout.fragment_buddha_assign, container, false)
        ...
        txtSearchTitle.text = "ค้นหาลูกค้าเพื่อส่งคาถา"
    ...
    private fun displayUsers(users: List<UserZ>) {
        val adapter = FindUserAdapter(users, object : FindUserAdapter.OnItemClickListener{
            override fun itemClick(userZ: UserZ) {
                // Redirect to SpellAssignPickerAct
                val intent = Intent(context, SpellAssignPickerAct::class.java)
                intent.putExtra("user_z", userZ)
