...
class AdminController extends Manager
{
    ...
    {
        $sql = "SELECT newsid as art_id, news_slug as slug, news_headline as title, news_desc as excerpt, category_name as category, news_pic_header as image_url, news_date as published_at, news_state as is_published, news_detail as content, news_title_short as title_short, fix as pin_order FROM news ORDER BY newsid DESC";
        ...
        foreach ($articles as &$article) {
            ...
            if (!empty($article['image_url']) && strpos($article['image_url'], 'http') !== 0) {
                $article['image_url'] = $host . '/' . ltrim($article['image_url'], '/');
            }
            // Fix Body/Content images URL
    ...
    public function saveArticleJson($request, $response)
    {
        ...
        if ($id) {
            $sql = "UPDATE news SET news_slug=:slug, news_headline=:title, news_desc=:excerpt, category_name=:category, news_detail=:content, news_state=:is_published, news_title_short=:title_short, news_pic_header=:image_url WHERE newsid=:id";
            $stmt = $this->db->prepare($sql);
            $success = $stmt->execute([
                ':slug' => $slug,
                ':title' => $title,
            ...
            $sql = "INSERT INTO news (news_slug, news_headline, news_desc, category_name, news_detail, news_state, news_date, news_title_short, news_pic_header) 
