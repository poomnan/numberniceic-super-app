...
class MeritController extends Manager
{
            ...
            if ($id >= 1 && $id <= 8) {
                ...
                if ($pang) {
                    $item = [
                        ...
                        'image' => $this->resolveMeritImage($pang->image_url),
                ...
                $item = [
                    ...
                    'image' => $this->resolveMeritImage('/uploads/buddha/default.png'),
    ...
    private function resolveMeritImage($imageUrl)
    {
        $value = trim((string) $imageUrl);
        if ($value === '') {
            return "http://43.228.85.200:81/uploads/buddha/default.png";
        }
        if (strpos($value, 'http://') === 0 || strpos($value, 'https://') === 0) {
            return $value;
        }
        return "http://43.228.85.200:81/" . ltrim($value, '/');
    }
    ...
    public function assignToUser($request, $response)
    {
        ...
        $dbError = null;
        try {
            // Check for duplicate using body (specific content), not title (generic label)
            // Multiple items can share same title (e.g. "ขั้นตอนการเปลี่ยนแปลง") but different body
            $chk = $this->db->prepare("SELECT id FROM user_merit_assign WHERE memberid = :mid AND merit_type = :type AND body = :body");
            $chk->execute([':mid' => $memberid, ':type' => $type, ':body' => $msgBody]);
            ...
            if ($existing) {
                ...
                $dbSaved = $ok;
                error_log("[MeritController] UPDATE id={$existing->id} member=$memberid type=$type ok=$ok");
            } else {
                // Insert
                ...
                $dbSaved = $ok;
                error_log("[MeritController] INSERT member=$memberid type=$type insertedId=$insertedId ok=$ok");
            }
        } catch (\PDOException $e) {
            $dbError = $e->getMessage();
            error_log("[MeritController] DB ERROR: " . $e->getMessage());
        }
        ...
        // Always return success with DB result info
        $result = [
            'status' => $dbSaved ? 'success' : ($dbError ? 'db_error' : 'unknown'),
            'db_saved' => $dbSaved,
        ];
        if ($dbError)
