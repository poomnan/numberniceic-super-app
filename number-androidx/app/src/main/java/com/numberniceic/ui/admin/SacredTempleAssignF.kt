...
class SacredTempleAssignF : Fragment() {
    ...
    companion object {
        fun newInstance(): SacredTempleAssignF {
            val args = Bundle()
            val fragment = SacredTempleAssignF()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        txtSearchTitle.text = "ค้นหาลูกค้าเพื่อแนะนำวัดศักดิ์สิทธิ์"
    ...
    private fun displayUsers(users: List<UserZ>) {
        val adapter = FindUserAdapter(users, object : FindUserAdapter.OnItemClickListener{
            override fun itemClick(userZ: UserZ) {
                // Here we redirect to SacredTemplePickerAct
                val intent = Intent(context, SacredTemplePickerAct::class.java)
