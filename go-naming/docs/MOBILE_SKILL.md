...
- สำหรับ input สองคำในวรรคเดียว ให้ตรวจ token แต่ละคำกับฐานข้อมูลก่อน semantic signal ถ้าคำแรกหรือคำที่สองมีในฐานข้อมูล ให้เป็น `full_name`
- `full_name` จะส่ง `seed_name` เป็นคำแรก และส่ง `semantic_query` เป็น input เต็ม เพื่อให้ระบบ decode ชื่อต้นแบบ แต่ค้น semantic จากชื่อ+สกุลทั้งชุด
