...
class BuddhaPangController extends Manager
{
    private function checkAdmin($response)
    {
        ...
        if (!$user || !isset($user->vipcode) || (strtolower($user->vipcode) !== 'admin' && strtolower($user->vipcode) !== 'administrator')) {
    ...
    public function viewEdit($request, $response, $args)
    {
        ...
        if (!$item) {
            return $response->withHeader('Location', '/web/admin/buddha')->withStatus(302);
        }
        ...
        if (isset($files['image_file'])) {
            ...
            if ($uploadedFile->getError() === UPLOAD_ERR_OK) {
                ...
                if (is_writable($directory)) {
                    ...
                    $image_url = '/public/uploads/buddha/' . $filename;
                } else {
    ...
    public function assignToUser($request, $response)
    {
        ...
        // Check if already assigned for this type
        $stmtCheck = $this->db->prepare("SELECT buddha_id FROM user_buddha_assign WHERE memberid = :mid AND assignment_type = :type");
        $stmtCheck->execute(['mid' => $memberid, 'type' => $type]);
        $exists = $stmtCheck->fetch();

        if ($exists) {
            $sql = "UPDATE user_buddha_assign SET buddha_id = :bid, custom_description = :desc, assigned_at = NOW() 
                    WHERE memberid = :mid AND assignment_type = :type";
        } else {
    ...
    public function getAssigned($request, $response, $args)
    {
        ...
        if (empty($result['annual'])) {
            ...
            $item->image_url = "/public/uploads/buddha/default.png";
        ...
        if (empty($result['lifetime'])) {
            ...
            $lifeItem->image_url = "/public/uploads/buddha/default.png";
    ...
    public function viewAssign($request, $response)
    {
        ...
        if (!empty($search)) {
            $sql = "SELECT memberid, username, realname, surname, birthday FROM membertb 
            ...
            $sql = "SELECT memberid, username, realname, surname, birthday FROM membertb 
                    ORDER BY memberid DESC LIMIT 10";
            $stmt = $this->db->query($sql);
            $users = $stmt->fetchAll(PDO::FETCH_OBJ);
        }

        // Get all Buddha Pangs for the dropdown
        $stmtPangs = $this->db->query("SELECT id, pang_name, buddha_day FROM buddha_pang_tb ORDER BY buddha_day ASC, id ASC");
        $pangs = $stmtPangs->fetchAll(PDO::FETCH_OBJ);

        return $this->container->get('view')->render($response, 'web_admin_buddha_assign.php', [
            'users' => $users,
            'search' => $search,
            'pangs' => $pangs
        ]);
    ...
    public function saveAssignmentWeb($request, $response)
    {
        $this->checkAdmin($response);
        $body = $request->getParsedBody();
        $memberid = $body['memberid'] ?? null;
        ...
        if (!$memberid || !$buddha_id) {
            return $response->withHeader('Location', '/web/admin/buddha/assign')->withStatus(302);
        }
        ...
        if (session_status() == PHP_SESSION_NONE)
            session_start();
        $_SESSION['status'] = ['type' => $success ? 'success' : 'danger', 'message' => $success ? 'มอบพระประโคมเรียบร้อยแล้ว' : 'เกิดข้อผิดพลาดในการบันทึก'];

        return $response->withHeader('Location', '/web/admin/buddha/assign' . (!empty($body['search']) ? '?search=' . urlencode($body['search']) : ''))->withStatus(302);
    }

    public function getHistoryApi($request, $response, $args)
    {
        $memberid = $args['memberid'];
        $sql = "SELECT a.*, b.pang_name FROM user_buddha_assign a 
                JOIN buddha_pang_tb b ON a.buddha_id = b.id 
                WHERE a.memberid = :mid ORDER BY a.assigned_at DESC";
        $stmt = $this->db->prepare($sql);
        $stmt->execute(['mid' => $memberid]);
